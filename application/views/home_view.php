...
<head>
	...
	<title>Expose a Cheater | Tellitize</title>


    <meta name="description" content="Anonymously send emails, expose cheaters, reveal crushes, apologize and more! Nobody will ever know your identity unless you want them to.">

    <meta name="keywords" content="expose a cheater,reveal a crush,anonymous sms, anonymous text,">
	<link rel="canonical" href="www.tellitize.com/" />
				...
				<div class="left">
						...
						<div class="top_bar">
							...
							<div class="mid-text">
								...
								<h3>Expose a Cheater 100% Anonymously</h3>

								<h4 class="style1">Anonymously Create a Post About Somebody</h4>
						...
						<div class="post_main">
							...
							<div class="post_main_content">
								...
								<?php } ?>
									...
									<textarea class="tetlize-text-comment" name="commentArea" id="commentArea" />Type Message Here</textarea>
							...
							*/
								?>
								 ...
								 <div  class="top_from" style="width:100%;margin-left:<?php echo $margin_left;?>;"><!--margin-bottom:3px;-->
								...
								<img width="90" height="26" alt="post-comment" src="<?php echo $GLOBALS['base_url']; ?>images/postviaemail.png"

								onClick="validatePostComment('post')" >

								</a>
                                                               <a href="javascript:void(0)">

								<img width="90" height="26" alt="sms-comment" src="<?php echo $GLOBALS['base_url']; ?>images/postviatxt.png"

								onClick="validatePostComment('sms')" >

								</a>
								</div>

								<div style="margin-left:0px;" class="postTopBox"><div class="postTop-title">SELECT CATEGORY: </div>
	...
	<div class="pop_up_bg"	>
       ...
       <form name="mailAlert" method="post" id="mailAlert">
        ...
        <div id="email_add_content">
            ...
            <h3> <span id="h3LablelId"> Anonymously email this message to somebody after posting it?</span>
            ...
            <div style="display:none" id="email_field">
                ...
                <li style="float:left">Email Id to Send To</li>

                <li id="IdForEmailorSms" style="float:left;padding-left:15px;">
            ...
            <!--</div>  extra div blocked by SB on 12-11-2014 -->
	...
	<div class="pop_up_bg">
       ...
       <form name="mailFrom" method="post" id="mailFrom">
        ...
        <div id="email_add_content">
            ...
            <h3>Anonymously email this message? Email will be sent from Tellitize.com
...
<script language="javascript">
	...
	$(document).ready(function() {
		 ...
		 $('input[id=email_yes]').change(function(){
                     // Check post Type Added by SB on 12-11-2014 start
                     //alert('Yes for == '+$('#post_type').val());
                    if($('#post_type').val()=='sms')
                    {
                        // email_field
                        $('#email_field > ul > li:first').html('Phone No.');
                        $('#IdForEmailorSms').html('<input type="text" id="email" name="email" value="" >[ Include Country code eg. +12094056778 ]');
                        $('#email_field > ul > li:last').html('<input type="button" value="Send Sms" onClick="sendAlert()" id="postbtn">');
                    }
                    else
                    {
                         $('#email_field > ul > li:first').html('Email Id to Send To');
                         $('#IdForEmailorSms').html('<input type="text" id="email" name="email" value="Enter email address here" onFocus="if(this.value  == \'Enter email address here\') { this.value = \'\'; } " onblur="if(this.value == \'\') { this.value = \'Enter email address here\'; } ">');
                         $('#email_field > ul > li:last').html('<input type="button" value="Send Alert" onClick="sendAlert()" id="postbtn">');
                   }
                   // Check post Type Added by SB on 12-11-2014 start
...
function agreePost(id){
			...
			if(data.success == "yes")
			{
				...
				$('.agree_'+id).attr('class', 'disableButton agree_'+id);
				$('.agree_'+id).html("");
				$('.agree_'+id).html("<input type='button' value='agree' class='disableButton-right'>");

				$('.disagree_'+id).attr('class', 'orangeButton disagree_'+id);
				$('.disagree_'+id).html("");
				$('.disagree_'+id).html("<input type='button' value='disagree' class='orangeButton-right' onclick='disagreePost("+id+");' >");
			...
			if(data.success == "yes")

			  {
				...
				*/
				...
				$('.agree_'+id).attr('class', 'orangeButton agree_'+id);
				$('.agree_'+id).html("");
				$('.agree_'+id).html("<input type='button' value='agree' class='orangeButton-right' onclick='agreePost("+id+");' >");

				$('.disagree_'+id).attr('class', 'disableButton disagree_'+id);
				$('.disagree_'+id).html("");
				$('.disagree_'+id).html("<input type='button' value='disagree' class='disableButton-right'>");
