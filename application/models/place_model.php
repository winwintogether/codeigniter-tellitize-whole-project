...
class Place_model extends CI_Model{
	...
	function getUserList($id){
		$podUserid=$this->getPodOwner($id);
		if($podUserid==$_SESSION['userid']) $delete=1;
		else $delete=0;
		$html='<ul>';		
		$query = $this->db->query("SELECT id,name,emailid,userid from discussionplace_userlist where placeid = ".$id." ORDER BY name ASC");
		foreach ($query->result_array() as $row)
			{
				...
				if($row['emailid']!='') 
				{
					$query = $this->db->query("SELECT userid,name from  users where email='".$row['emailid']."'");
					 if ($query->num_rows()){
						...
						//$user_link=$GLOBALS['base_url'].'view-profile/'.$name;
				...
				if($full_name=='') $full_name=$row['name'];
				 $html.='<li id="poduserlist_'.$row['id'].'">
								...
								<a href="javascript:void(0)" class="postimg" onClick="showPostsAbout('.$pod_username.','.$pod_useremail.','.$id.');">
	...
	function getPodOwner($id){
		$query = $this->db->query("SELECT userid from place_of_discussion where id=".$id);
		foreach ($query->result_array() as $row){
			$podUserid=$row['userid'];
		}
		return $podUserid;
	}
	function getAllUserList($id){
		...
		$query = $this->db->query("SELECT name,emailid from discussionplace_userlist where placeid=".$id);
		foreach ($query->result_array() as $row)
			{
	...
	function article($id){
		$html='';
			...
			$query = $this->db->query(
					...
					where userid =".$_SESSION['userid'].") AND  placeid=".$id." and status=0 order by  postid desc limit 10
					");
	       ...
	       if($category=='') $style='style="padding-top:6px"';
			...
			$category_link= str_replace(" ","-",$category);
			 ...
			 $postuser_img='';
					...
					$comment_post_pod= $this->home_model->autolink($row['comment']);				
