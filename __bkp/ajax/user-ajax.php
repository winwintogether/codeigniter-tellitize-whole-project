				...
				if($row_cat=mysql_fetch_array($selectcat)){					
					...
					if(isset($_POST['group']) || isset($_POST['place'])){
						...
						if($in){
								...
								$view_url=$base_url.''.$cat_name.'/'.preg_replace("![^a-z0-9]+!i", "-", $title_post).'/'.$post_id;
...
if(isset($_GET['mode']) AND $_GET['mode']=='saveDiscussionUser'){
	...
	if(isset($userid) && $userid==0){
			...
			if(isset($_POST['email']) && $_POST['email']!='')
			{
			 ...
			 $selectexistname=mysql_query("SELECT count(*) as cnt from discussionplace_userlist 
		   where name='".$_POST['name']."' AND placeid='".$_POST['places']."'");
			if($r_name=mysql_fetch_array($selectexistname)){
					if($r_name['cnt']>0){
						$val = array("success" => "no","status" =>"existname");
										$output = json_encode($val);
