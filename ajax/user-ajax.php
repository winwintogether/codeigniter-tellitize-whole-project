...
if(isset($_GET['mode']) AND $_GET['mode']=='sendSms'){
    $comment_posted = htmlspecialchars($_POST['commentArea']);
            ...
            $insert=mysql_query($in);
				...
				if($row_cat=mysql_fetch_array($selectcat)){
					...
					if(isset($_POST['place_name']) and $_POST['post_about']=='place'){
						...
						if(isset($_POST['email'])){
								//$smsMessage=str_replace(" ","-",$_POST['other_desc']);
                                                       // $smsMessage=$_POST['other_desc'];
                                                        $smsMessage=trimText('View at www.tellitize.com. '.$_POST['commentArea'], 160);                                                        
							//$msisdn = $_POST['email']; // the phone no for message
                                                        $msisdn = str_replace(" ","",$_POST['email']); // the phone no for message
                                                        //$smsMessage='test here again';
                                                        //$msisdn ='18596409345';
							$smsusername = 'Telli1258';
                                                        $smspass = 'mtdew1258';
						   if($msisdn!=''){
                                                       // send sms code start                                                         
                                                      ...
                                                      // $response = do_post_request($url, $data);
                                                       ...
                                                       // send sms code end
								if($response){
								$insert=mysql_query("INSERT INTO `sms_status` (`userid`,`phone_no`,`comment`,`status`)	VALUES ('".$_SESSION['userid']."','".$msisdn."','".$smsMessage."','1')");
								if($insert) $mailsuccess='smssent';
							}else $mailsuccess='smsfailed';
						}
						else $mailsuccess='smsfailed';
						}
					}
					...
					$val = array("success" => "yes","status" =>$insert,"sms"=>$mailsuccess,"result"=>$result);
					$output = json_encode($val);
					echo $output;
				}
				else{
...
// function for sms sending  Start
function do_post_request($url, $data, $optional_headers = 'Content-type:application/x-www-form-urlencoded') {
 ...
 // function for sms sending  End
 //function for text trim to required size start 
...
//function for text trim to required size end