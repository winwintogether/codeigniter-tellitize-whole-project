...
if(isset($_GET['mode']) AND $_GET['mode']=='reportAbuse'){
	...
	$comment_posted = htmlentities($_POST['comment']);
        ...
        $anonymous_user = $_POST['anonymous_user'];
	...
	$in="INSERT INTO  post_replies (`id`, `postid`, `userid`, `comment`, `date`) 

             VALUES (NULL ,'".$_POST['postid']."','".$userid."','".$comment_posted."','".date("y/m/d")."')";
	...
	//$id=mysql_insert_id();												  
        ...
        if(mysql_query($in)) {
            ...
            if($userid!=$_SESSION['userid'])
                ...
                $Querynotify=mysql_query("INSERT INTO  notifications (`id`, `notify_id`, `notification_on`, `userid`, `date`)

					  VALUES (NULL ,'".$notifyid."','reply','".$userid."','".date("y/m/d")."')");
		...
		while($row1=mysql_fetch_array($selectreplies)){
...

                    {
                            ...
                            $delete_link = '<div class="deleteMsg deletReply'.$row1['id'].'">
                                                                    ...
                                                                    <input type="button" onclick="deleteReply('.$row1['id'].','.$_POST['postid'].');" style="margin-right:-18px;" value="DELETE" class="deleteMsg-right">
...

                    {
...

                            {
                                    ...
                                    /*$agree_link	= '<img border="0" alt="Agreed" src="http://localhost/tellitize_new//images/agree.png">';

                                    $disagree_link	= '<input type="button" value="disagree" class="agreeDisagree" onclick="disagreeReply('.$row1['id'].');" >';*/
...

                            {
                                    ...
                                    /*$agree_link	= '<input type="button" value="agree" class="agreeDisagree" onclick="agreeReply('.$row1['id'].');">';

                                    $disagree_link	= '<img border="0" alt="Agreed" src="http://localhost/tellitize_new//images/disagree.png">';*/
                    ...
                    $html.='<div class="replyComment'.$row1['id'].'" id="replyComment">
                                    ...
                                    <div class="postcontent-aggre-main">
                                            ...
                                            <div class="circle" id="agreereply_circle'.$row1['id'].'">'.$agree_cnt_post.'</div>

                                            <div style="font-size:10px;font-size:10px;font-weight:normal;color:#666666; padding:4px 23px 0px 4px; float:left;">&nbsp; AGREE</div>

                                            <div class="circle" id="disagreereply_circle'.$row1['id'].'">'.$disagree_cnt_post.'</div>
            ...
            /* Email to the user starts*/



            $query  = " SELECT U.name, U.last_name, U.email, PP.userid FROM public_post AS PP 

                        LEFT JOIN users AS U ON PP.userid = U.userid WHERE PP.postid = '".$_POST['postid']."'";
            ...
            $query2         = " SELECT PD.first_name, PD.last_name, PD.place, PP.comment, C.cate_name, PD.other_description FROM public_post AS PP 

                                LEFT JOIN category AS C ON PP.cid = C.cid LEFT JOIN post_details AS PD ON PP.postid = PD.postid WHERE PP.postid = '".$_POST['postid']."'";

            $selectQuery2   = mysql_query($query2);

            $arr_query2     = mysql_fetch_array($selectQuery2);

            

            $cat_name       = str_replace(" ","-",$arr_query2['cate_name']);

            $cat_name       = preg_replace("![^a-z0-9]+!i", "-",$cat_name );



            if($arr_query2['other_description'] != '') 

            {

                $title_post     = $arr_query2['other_description'];

            }

            else if($arr_query2['place'] != '')

            {

                $title_post     = $arr_query2['place'];
...

            {
                ...
                $title_post     = $arr_query2['first_name']." ".$arr_query2['last_name'];
            ...
            $view_url       = $base_url.''.$cat_name.'/'.preg_replace("![^a-z0-9]+!i", "-", $title_post).'/'.$_POST['postid'].'/ref';
            ...
            $msg            = "<a href='".$base_url."' title='tellitize.com'>
                               ...
                               </a><br><br>Hello ".$name.", <br><br>Somebody has responded to your post. 
...
if(isset($_GET['mode']) AND $_GET['mode']=='deleteReply'){ 
	...
	$html='';
        ...
        /*$getQuery   = mysql_query("SELECT postid FROM post_replies where postid='".$_POST['pid']."'");

        $get_row    = mysql_fetch_array($getQuery);
	...
	//$delete=mysql_query("Delete from post_replies where id= 36");
		...
		while($row1=mysql_fetch_array($selectreplies)){
...

                {
                        ...
                        $delete_link = '<div class="deleteMsg deletReply'.$row1['id'].'">
                                                                ...
                                                                <input type="button" onclick="deleteReply('.$row1['id'].','.$_POST['pid'].');" style="margin-right:-18px;" value="DELETE" class="deleteMsg-right">
                        ...
                        $user_image.=$userPic;
										...
										$html.='<div class="replyComment'.$row1['id'].'" id="replyComment">
												...
												<div class="postcontent-aggre-main">
													...
													<div class="circle" id="agreereply_circle'.$row1['id'].'">'.$disagree_cnt_post.'</div>

													<div style="font-size:10px;font-size:10px;font-weight:normal;color:#666666; padding:4px 23px 0px 4px; float:left;">&nbsp; DISAGREE</div>	

													<div style=" float:right; margin:0 12px 6px 0;">

														<div style="float:left; margin:0 10px 0 0;" class="agreeReply'.$row1['id'].'">'.$agree_link.'</div>

														<div style="float:left; margin:0 10px 0 0;" class="disagreeReply'.$row1['id'].'">'.$disagree_link.'</div>'.$delete_link.'
...
if(isset($_GET['mode']) AND $_GET['mode']=='viewPostsAbout'){
	...
	//$html='';
			...
			$people_disagree='DisAgree';
                        ...
                        $select=mysql_query("SELECT PP.postid, PP.comment, RAL.status FROM public_post AS PP LEFT JOIN post_details AS PD ON PP.postid = PD.postid

LEFT JOIN report_abused_list AS RAL ON PD.postid = RAL.postid WHERE PD.first_name = '".$to_user_name."' AND PD.post_about = 'pod'");
											...
											<div class="postcontent-main">
											 ...
											 <div class="postcontent">
											  ...
											  <div class="postcontent-aggre-main">
													 ...
													 <div class="circle" id="agreereply_circle'.$row['postid'].'">'.$agree_cnt.'</div>

													 <div class="people-agree">'.$people_agree.'</div>

													 <div class="circle" id="disagreereply_circle'.$row['postid'].'">'.$disagree_cnt.'</div>
					...
					$html.='<div id="replyComment">
							...
							<form name="replyPost" id="replyPost">
								...
								<div class="postreply" onclick="postReply('.$row['postid'].');">
									...
									<img alt="comment" src="'.$GLOBALS['base_url'].'images/comment.png">
		...
		$html='';
				...
				if(isset($_GET['postid']) ){
					...
					$html.='<ul><li id="email_left">To:</li><li id="email_rit"><input type="text" name="to_email" id="to_email"></li></ul>
						...
						<ul><li id="email_left">This message will be sent from info@tellitize.com so rest assured your identity will not be revealed! Feel free to test it out by sending this message to your personal email address first.</li>
...
if(isset($_GET['mode']) AND $_GET['mode']=='sendEmail'){
    ...
    if(isset($_GET['post_id'])){
			...
			//$view_url=$base_url.''.$cat_name.'/'.preg_replace("![^a-z0-9]+!i", "-", $title_post).'/'.$_GET['post_id'];
                        ...
                        $view_url=$base_url.''.$cat_name.'/'.preg_replace("![^a-z0-9]+!i", "-", $title_post).'/'.$_GET['post_id'].'/ref';
...
if(isset($_GET['mode']) AND $_GET['mode']=='postReplyPOD'){
    ...
    $in = "INSERT INTO  post_replies (`id`, `postid`, `userid`, `comment`, `date`)
            ...
            VALUES (NULL ,'".$_POST['postid']."','".$_SESSION['userid']."','".$comment_posted."','".date("y/m/d")."')";
	...
	if ($row_user=mysql_fetch_array($selectUser) ) {
