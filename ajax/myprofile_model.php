...
class Myprofile_model extends CI_Model{
	...
	function getgroup(){
		$html='';
		$query = $this->db->query("SELECT * FROM group_list where userid='".$_SESSION['userid'] ."'");
	...
	function getgroupList(){
		$html='';
		$query = $this->db->query("SELECT * from  group_list where id in (select groupid  from group_userlist where userid=".$_SESSION['userid'].")");
		
		foreach ($query->result_array() as $row)
		{
		 
		$html.='<li id="grouplist'.$row['id'].'" class="discussion"  onmouseover="showEdit('.$row['id'].');" onmouseout="hideEdit('.$row['id'].');">
					<a id="group'.$row['id'].'" >'.$row['group_name'].'</a>
					<span style="float:right;display:none" class="edit_delete'.$row['id'].'">
						<a href="'.$GLOBALS['base_url'].'index.php/group?group-id='.$row['id'].'"><img width="45" height="19" alt="edit" src="'.$GLOBALS['base_url'].'images/edit-btn.jpg"></a>
